...
//
// =====================================================================
// static mut 全局状态使用契约（与 main.rs 顶部"单线程不变量"配套）
// ---------------------------------------------------------------------
// C 的全局变量在 Rust 中没有直接对应物。本程序为单线程，因此使用
// `static mut` 保持与 C 完全一致的全局可变语义。所有访问必须满足：
//
//   1. 访问一律放在 unsafe 块内，并附中文注释说明读取/写入了哪个全局；
//   2. 每次访问是语句级短借用：读一个 Copy 字段、一次 as_mut()/take()，
//      不得把 &mut 存入局部变量后跨语句使用；
//   3. 不得在并发上下文（线程、回调、析构、信号处理）中访问任何
//      static mut——单线程是这些访问合法性的唯一前提；
//   4. 新增全局状态优先考虑 OnceLock/LazyLock/线程局部/参数传递，
//      确需 static mut 时集中在本文件声明；
//   5. 测试访问全局时必须持有 TEST_LOCK 串行化（见本文件 TEST_LOCK）。
//
// 违反 1-3 即构成未定义行为（数据竞争或引用别名冲突）。
// =====================================================================
//
...
pub static VERSION: &str = concat!(
    ...
    " %s 2026 - by TC999; %s 1996 - 2026 by Steve Baker, Thomas Moore, Francesc Rocher, Florian Sesser, Kyosuke Tokoro $"
);

// C: char *hversion = "\t\t tree v2.3.2 %s ...";（HTML 输出版本脚注，html.c 引用）
pub static HVERSION: &str = concat!(
    "\t\t rt v",
    env!("CARGO_PKG_VERSION"),
    " %s 1996 - 2026 by Steve Baker and Thomas Moore <br>\n\
