...
// =====================================================================
// 单线程不变量（安全契约，修改本文件或任何 static mut 前必读）
// ---------------------------------------------------------------------
// 本程序自 main() 入口到 process::exit 全程单线程顺序执行，这是所有
// unsafe 与 static mut 全局访问得以安全的前提。必须始终满足：
//
//   1. 不得引入并发执行（线程、async、信号处理等上下文中访问全局状态）；
//   2. 对 static mut 全局的每次访问必须是语句级短借用：读取 Copy 字段、
//      一次 as_mut()/take()；不得把 &mut 存入局部变量后跨语句使用；
//   3. 新增可变全局状态时优先考虑 OnceLock/线程局部/参数传递，
//      确需 static mut 时集中到 globals.rs 声明并遵循其使用契约；
//   4. 测试访问全局时必须持有 globals::TEST_LOCK 串行化。
//
// 违反 1-2 即构成未定义行为（数据竞争或引用别名冲突）。
// 详见 globals.rs 顶部模块文档。
// =====================================================================

// 允许 static_mut_refs：本程序为单线程（见上方"单线程不变量"），所有对
// static mut 全局变量的访问均在 unsafe 块内并附中文注释，语义与 C 的
// 全局变量一致；直接对 static mut 取引用的方法调用（如 STATIC.take()）
// 因此被允许。
